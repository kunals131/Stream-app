...
export const createStream = formValues=> async (dispatch, getState)=>{
    ...
    const res = await streams.post('/streams', JSON.stringify({...formValues, userId}));
    dispatch({type : CREATE_STREAM, payload : res.data});
    //Do some programmatic navigation to get user to the root route
    history.push('/');
...
export const editStream = (id,formValues)=>async dispatch=>{
    const res = await streams.patch(`/streams/${id}`, JSON.stringify(formValues));
